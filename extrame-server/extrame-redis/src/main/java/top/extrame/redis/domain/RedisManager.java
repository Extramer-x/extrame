package top.extrame.redis.domain;
...
import top.extrame.redis.config.RedisProperties;
...
public class RedisManager {
    ...
    private void processTemplateRegistry(ConfigurableApplicationContext applicationContext) {
        ...
        redisProperties.getDatabases().forEach((name, index) -> {
            if (!Objects.isNull(index)){
                ...
                builder.addConstructorArgValue(reactiveRedisConnectionFactory(index));
     ...
     */
    protected LettuceConnectionFactory reactiveRedisConnectionFactory(Integer database) {
